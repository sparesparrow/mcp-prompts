# MCP Prompts Server - Architecture Diagrams
...
graph TB
    subgraph "Source Code Structure"
        ...
        subgraph "Utilities"
            ...
            TEST[src/index.test.ts<br/>Tests]
        end
    end
    
    subgraph "Configuration"
        PKG[package.json<br/>Package Config]
        TS_CONFIG[tsconfig.server.json<br/>TypeScript Config]
        DOCKER[Dockerfile.mcp<br/>Docker Config]
        COMPOSE[docker-compose.mcp.yml<br/>Docker Compose]
    end
    
    subgraph "Data & Documentation"
        SAMPLE[data/sample-prompts.json<br/>Sample Templates]
        README[README.md<br/>Main Documentation]
        MCP_README[MCP_README.md<br/>MCP Guide]
        RELEASE[RELEASE_NOTES_v3.0.8.md<br/>Release Notes]
    end
...
graph TD
    subgraph "Development Process"
        A[Feature Development] --> B[TypeScript Compilation]
        B --> C[Testing]
        C --> D[Documentation]
        D --> E[Git Commit]
        E --> F[Build Package]
    ...
    subgraph "Quality Gates"
        B --> B1[TypeScript Errors?]
        C --> C1[Tests Pass?]
        F --> F1[Build Success?]
        G --> G1[Publish Success?]
    end
...
4. **Component Structure** - File organization and dependencies
5. **Template Processing** - Variable substitution workflow
6. **Docker Deployment** - Containerization strategy
7. **Version Evolution** - Project development timeline
8. **Tool Interactions** - MCP tool communication flow
9. **Use Cases** - Real-world application scenarios
10. **Data Model** - Entity relationships and structure
11. **Development Workflow** - Process and quality gates

The diagrams demonstrate the "show over tell" approach by visualizing the complex architecture and relationships rather than just describing them in text.
