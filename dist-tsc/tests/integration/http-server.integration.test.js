...
let promptService;
let adapter;
const SAMPLE_WORKFLOW_PATH = path.resolve(process.cwd(), 'data', 'workflows', 'sample-workflow.json');
...
beforeAll(async () => {
    ...
    const sequenceService = new DummySequenceService();
    const workflowService = new WorkflowService(adapter, promptService);
    server = await startHttpServer(null, { host: '127.0.0.1', port: 0 }, {
        promptService,
        sequenceService,
...
describe('Prompt List (GET /prompts)', () => {
    ...
    it('should sort by name ascending', async () => {
        const res = await request(baseUrl)
            ...
            .set('x-api-key', 'test-key');
        expect(res.status).toBe(200);
        const names = res.body.prompts.map((p) => p.name);
...
describe('Bulk Prompt Operations', () => {
    ...
    it('should bulk create prompts successfully, handling duplicates and invalids', async () => {
        ...
        expect(results.some((r) => r.success === false && r.error.includes('already exists'))).toBe(true);
        expect(results.some((r) => r.success === false && r.error.includes('Invalid prompt data'))).toBe(true);
        expect(results.filter((r) => r.success === true).length).toBe(2);
    ...
    it('should bulk delete prompts and return per-id results', async () => {
        const created = (await request(baseUrl)
            .post('/prompts/bulk-create')
            .set('x-api-key', 'test-key')
            .send({ prompts: [makePromptPayload({ name: 'p1' }), makePromptPayload({ name: 'p2' })] })).body.results;
        const res = await request(baseUrl)
        ...
        expect(res.body.results.find((r) => r.id === created[0].id)?.success).toBe(true);
        expect(res.body.results.find((r) => r.id === 'non-existent')?.success).toBe(false);
    });
...
function makePromptPayload(overrides = {}) {
