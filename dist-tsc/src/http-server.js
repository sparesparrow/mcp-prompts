...
import { auditLogWorkflowEvent, getWorkflowRateLimiter, releaseWorkflowSlot, } from './workflow-service.js';
...
function getWorkflowFileName(id, version) {
...
function saveWorkflowToFile(workflow) {
    ...
    fs.writeFileSync(getWorkflowFileName(workflow.id, workflow.version), JSON.stringify(workflow, null, 2));
}
function loadWorkflowFromFile(id, version) {
    ensureWorkflowDir();
...
function getAllWorkflowVersions(id) {
    ensureWorkflowDir();
    const files = fs.readdirSync(WORKFLOW_DIR)
        .filter(f => f.startsWith(`${id}-v`) && f.endsWith('.json'));
    return files
 ...
 * @param services
 */
export async function startHttpServer(server = null, config, services) {
    const app = express();
     ...
     *                   type: integer
     */
    app.get('/prompts', catchAsync(async (req, res, next) => {
        ...
        const options = { offset, limit, sort, order, category, isTemplate, search };
        if (tags) {
     ...
     */
    app.post('/prompts', catchAsync(async (req, res, next) => {
     ...
     */
    app.get('/prompts/:id', catchAsync(async (req, res, next) => {
    ...
    app.put('/prompts/:id/:version', catchAsync(async (req, res, next) => {
        const prompt = await services.promptService.updatePrompt(req.params.id, parseInt(req.params.version, 10), req.body);
        if (prompt) {
     ...
     */
    app.get('/prompts/:id/versions', catchAsync(async (req, res, next) => {
     ...
     *                     type: string
     */
    app.post('/prompts/bulk-create', catchAsync(async (req, res) => {
     ...
     *                     type: string
     */
    app.post('/prompts/bulk-delete', catchAsync(async (req, res) => {
        const { ids } = req.body;
    ...
    app.post('/diagram', catchAsync(async (req, res, next) => {
        ...
        const prompts = await Promise.all(promptIds.map((id) => services.promptService.getPrompt(id)));
     ...
     */
    app.post('/api/v1/workflows', catchAsync(async (req, res, next) => {
     ...
     */
    app.get('/api/v1/workflows/:id', catchAsync(async (req, res, next) => {
     ...
     */
    app.get('/api/v1/workflows/:id/versions', catchAsync(async (req, res, next) => {
     ...
     * Response: { ...workflow } or 404
     */
    app.get('/api/v1/workflows/:id/versions/:version', catchAsync(async (req, res, next) => {
     ...
     * Response: { success, id, version, message }
     */
    app.delete('/api/v1/workflows/:id/versions/:version', catchAsync(async (req, res, next) => {
        const version = parseInt(req.params.version, 10);
        if (isNaN(version)) {
            res.status(400).json({
                success: false,
     ...
     * Response: [ { ...workflow }, ... ]
     */
    app.get('/api/v1/workflows', catchAsync(async (req, res, next) => {
     ...
     */
    app.post('/api/v1/workflows/:id/run', catchAsync(async (req, res, next) => {
     ...
     *                   type: string
     */
    app.post('/api/v1/workflows/:executionId/resume', catchAsync(async (req, res, next) => {
