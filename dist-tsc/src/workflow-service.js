...
export class WorkflowServiceImpl {
    storageAdapter;
    promptService;
    constructor(storageAdapter, promptService) {
     ...
     */
    async runWorkflowSteps(workflow, state, stepRunners) {
        ...
        try {
            while (currentStepId) {
                ...
                if (step.type === 'parallel') {
                    ...
                    try {
                        const results = await Promise.all(parallelSteps.map(subStep => this.runSingleStep(subStep, context, stepRunners)));
            ...
            console.error(`Workflow ${state.workflowId} failed during execution ${state.executionId}`, err);
    ...
    async runSingleStep(step, context, stepRunners) {
    ...
    async runWorkflow(workflow, initialContext = {}) {
        const userId = initialContext.userId || 'anonymous';
    ...
    async getWorkflowById(workflowId) {
        ...
        if (typeof this.storageAdapter.getWorkflow === 'function') {
            return await this.storageAdapter.getWorkflow(workflowId);
        }
...
export class PromptRunner {
    ...
    async runStep(step, context) {
        const timeoutMs = typeof step.timeout === 'number' ? step.timeout : 60000;
        return withTimeout(this._runStep(step, context), timeoutMs);
    }
    async _runStep(step, context) {
    ...
    async runStep(step, context) {
        ...
        try {
            if (sandboxImage) {
                ...
                console.warn('[SECURITY WARNING] ShellRunner is not sandboxed! This is unsafe for production. Set SHELL_SANDBOX_IMAGE to enable Docker-based sandboxing.');
...
export class HttpRunner {
    ...
    async _runStep(step, context) {
        ...
        const { method, url, body, headers, auth } = step;
        // Support headers, auth, advanced options
        const fetchHeaders = {
            'Content-Type': 'application/json',
            ...(headers || {}),
        };
        let fetchOptions = { headers: fetchHeaders, method };
        if (body)
...
export function getWorkflowRateLimiter() {
    return (userId) => {
        if (!workflowConcurrency[userId])
            workflowConcurrency[userId] = 0;
        ...
        console.debug(`[RateLimiter] After increment: userId=${userId}, count=${workflowConcurrency[userId]}`);
        return true;
    };
}
/**
 ...
 * @param userId
 */
export function releaseWorkflowSlot(userId) {
    if (workflowConcurrency[userId])
        workflowConcurrency[userId]--;
    console.debug(`[RateLimiter] After release: userId=${userId}, count=${workflowConcurrency[userId]}`);
}
// --- Audit Logger ---
