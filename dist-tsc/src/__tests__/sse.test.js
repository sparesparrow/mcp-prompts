...
describe('SseManager', () => {
     ...
     * @param es
     */
    async function closeEventSource(es) {
    ...
    if (typeof SseManager.prototype['addClient'] === 'function') {
        const origAddClient = SseManager.prototype['addClient'];
        SseManager.prototype['addClient'] = function (...args) {
            const result = origAddClient.apply(this, args);
            console.log('[SSE TEST] SseManager.addClient called. Client IDs:', this.getClientIds());
            return result;
        };
    }
    if (typeof SseManager.prototype['removeClient'] === 'function') {
        const origRemoveClient = SseManager.prototype['removeClient'];
        SseManager.prototype['removeClient'] = function (...args) {
    ...
    it('should handle multiple clients', async () => {
        ...
        await new Promise((resolve, reject) => {
            ...
            const messageHandler = (esNum) => (event) => {
                if (resolved)
                    return;
                const data = event.data;
    ...
    it('should handle client errors', async () => {
        if (process.env.SKIP_FLAKY_SSE_ERROR_TEST) {
            console.warn('[SSE TEST] Skipping flaky SSE error test due to SKIP_FLAKY_SSE_ERROR_TEST env var');
            return;
        }
