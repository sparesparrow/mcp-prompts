...
const PromptSchema = z.object({
  ...
  variables: z.array(z.union([z.string(), z.object({
    name: z.string(),
...
function loadSamplePrompts() {
  try {
    const sampleDataPath = join(process.cwd(), 'data', 'sample-prompts.json');
...
export async function createMcpServer() {
  ...
  server.tool('add_prompt', async (params: any) => {
    try {
  ...
  server.tool('get_prompt', async (params: any) => {
    try {
  ...
  server.tool('list_prompts', async (params: any = {}) => {
    try {
  ...
  server.tool('update_prompt', async (params: any) => {
    try {
  ...
  server.tool('delete_prompt', async (params: any) => {
    try {
      const { id } = z.object({ id: z.string() }).parse(params);
      const prompt = prompts.get(id);
      if (!prompt) {
  ...
  server.tool('apply_template', async (params: any) => {
    try {
