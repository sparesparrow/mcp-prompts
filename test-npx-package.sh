...
echo "Testing @sparesparrow/mcp-prompt-manager via npx in a clean environment..."
...
if command -v npx @modelcontextprotocol/inspector &> /dev/null; then
  ...
  timeout 10s npx @modelcontextprotocol/inspector npx -y @sparesparrow/mcp-prompt-manager || {
...
echo "The package can be run with: npx -y @sparesparrow/mcp-prompt-manager" 